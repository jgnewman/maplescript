...
MapleScript may not be Canadian, but it _can_ be used with any JavaScript library. The language compiles to JavaScript and contains a built-in event system, a virtual DOM, and a JSX-like syntax for creating html, all in about 24kb of minified overhead.
...
In MapleScript, all expressions return values and, aside from raw data, just about everything is syntactically structured like a function call. Function calls are just lists of values enclosed in parentheses ("s-expressions"). Here are a few MapleScript expressions and their JavaScript equivalents:
...
# do some stuff and then recurse.
(make factorial
  (of [0] 1)
